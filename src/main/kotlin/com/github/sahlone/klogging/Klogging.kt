...
data class TracingContext(val correlationId: String = "${UUID.randomUUID()}") : JsonDeserializer<TracingContext>() {
    ...
    override fun deserialize(p: JsonParser?, ctxt: DeserializationContext?): TracingContext =
        Optional.ofNullable(p?.text).map { TracingContext(it) }.orElseGet {
            throw IllegalStateException("Couldn't deserialize tracing context.")
        }
}
...
class ContextualLogger<T>(private val delegate: Logger) {
    ...
    fun info(context: TracingContext, vararg additionalArgs: ContextualLoggerMarker, msg: () -> String) {
        if (delegate.isInfoEnabled) {
            delegate.info(generateMarker(context, additionalArgs.asList()), msg.invoke())
        }
    }
    ...
    fun infoWithCause(
        ...
        if (delegate.isInfoEnabled) {
            delegate.info(generateMarker(context, additionalArgs.asList()), msg.invoke(), cause)
        }
    }
